...
`register` / `ship` / `close` / `land` は、タスクを扱う場合に、タスクをどこで扱うかを導入先リポジトリの readme から読み取ります。
...
- `ship`: 実装・検証から Draft PR 作成まで進め、人間がレビューできるようそこで停止します。
- `close`: 対象 PR をマージします。
- `land`: 実装済みの変更を確認・検証し、PR 作成からマージまで行います。実装やコード修正は行いません。対象タスクがあれば「レビュー中」から「完了」へ更新し、タスクがなければ作成せず進めます。PR 作成後の追加のマージ確認は行いませんが、必須チェックや必須レビューは省略しません。修正が必要な場合は問題を報告して停止します。

例えば、レビューしてからマージしたい場合は「今の変更を ship」と依頼し、確認後に「close」と依頼します。
変更が確定していてマージまで任せたい場合は「今の変更を land」と依頼します。
