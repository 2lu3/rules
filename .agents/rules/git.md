...
- Explicitly invoking the `close` skill counts as explicit permission for merging the PR it targets and moving the task it closes to "done".
- Explicitly invoking `land`, or requesting PR creation and merge together, authorizes committing and delivering existing changes, merging the latest `main` into the feature branch, pushing, PR creation and merge, and associated task status updates. It does not authorize implementation or code fixes; no additional merge confirmation is required.
